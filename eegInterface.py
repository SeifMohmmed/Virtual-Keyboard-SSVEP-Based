...
checker_frequency = np.array([10, 20, 15, 5, 12])
...
# One key per box — pressing the key instantly selects that box
KEY_BOX_MAP = {
    'up':    0,
    'right': 1,
    'down':  2,
...
class BCI(object):
    def __init__(self, win_size, freq_array, checker_size, checker_cycles, checker_tex, pipe):
        ...
        self.win = visual.Window(self.win_size, units='pix', monitor='testMonitor')
        ...
        for i in range(len(self.top_level_boxes)):
            ...
            self.selection_boxes.append(visual.TextStim(self.win,
                                                        ...
                                                        pos=[self.stim[i].pos[0],
                                                             self.stim[i].pos[1] + 0.60 * self.checker_size]))
        ...
        self.entered_textbox = visual.TextStim(self.win,
                                               text='|',
                                               color=[-1, -1, -1],
                                               pos=[self.entered_background.pos[0],
                                                    self.entered_background.pos[1]],
                                               alignHoriz='center',
                                               alignVert='center')

        # Green box shown around the selected box
        ...
        # Yellow key labels drawn below each checker box
        key_labels = ['10hz', '20hz', '15hz', '5hz', '12hz']
        self.key_label_stims = []
        for i in range(self.num_of_stim):
            label = key_labels[i] if i < len(key_labels) else ''
            self.key_label_stims.append(visual.TextStim(
                self.win,
                text='[{}]'.format(label),
                color=[1, 1, -1],
                pos=[self.stim[i].pos[0], self.stim[i].pos[1] - 0.65 * self.checker_size],
                alignHoriz='center',
                alignVert='center',
                height=18
            ))
        ...
        print("Frame rate is {0}. Actual Flashing Frequency will be {1}".format(
            self.frame_rate, str(1 / (self.stim_frames * self.frame_interval))))
    ...
    def start(self):
        ...
        while not event.getKeys('escape'):
            ...
            if not self.setting_baseline:
                # Check every mapped key — one press = direct box selection
                keys = event.getKeys(list(KEY_BOX_MAP.keys()))
                for key in keys:
                    if self.selected_index == -1:           # ignore if already processing
                        box_index = KEY_BOX_MAP[key]
                        if box_index < self.num_of_stim:
                            self.selected_index = box_index
                            self.selected_box.pos = self.stim[self.selected_index].pos
                            self.selected_box_frames = self.selected_box_on_frames
                    break  # handle only one key per frame

                # Show green highlight and wait before processing the selection
        ...
        if not self.bottom_level:
            ...
            if len(self.selection_boxes[self.selected_index].text) > 1:
                if self.selection_boxes[self.selected_index].text == num_text:
                    ...
                    print('Unknown case, resetting to top level')
                    self.bottom_level = False
            ...
            if self.setting_baseline:
                ...
                if self.baseline_count == max_baseline_count:
                    for i in range(freq_array_len):
                        ...
                        print("freq {} mean {}, std {}, thresh {}".format(
                            self.freq_array[i], mean, std, freq_sig_val_thresh[i]))
